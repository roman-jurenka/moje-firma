  ...
  const checkin = async () => {
    ...
    if (todayRecord) {
      ...
      const { data: row } = await supabase.from("attendance")
        .upsert({ employee_id: myEmpId, date: todayStr, checkin: time, checkout: null, project_id: null, activity: null }, { onConflict: "employee_id,date" })
        .select().single();
...
function Attendance({ currentUser, attendance, setAttendance, employees }) {
  ...
  const checkinNow = async () => {
    ...
    if (todayRecord) {
      ...
      const { data: row } = await supabase.from("attendance")
        .upsert({ employee_id: viewEmpId, date: todayStr, checkin: time, checkout: null, project_id: null, activity: null }, { onConflict: "employee_id,date" })
        .select().single();
      if (row) setAttendance([...attendance, { ...row, employeeId: row.employee_id }]);
    }
  };

  const saveRecordDetail = async (id, projectId, activity) => {
    await supabase.from("attendance").update({ project_id: projectId || null, activity: activity || null }).eq("id", id);
    setAttendance(attendance.map(a => a.id === id ? { ...a, project_id: projectId, activity } : a));
    setEditRecord(null);
  };
  ...
  const addManual = async () => {
    ...
    if (existing) {
      await supabase.from("attendance").update({ checkin: manualIn, checkout: manualOut || null }).eq("id", existing.id);
      setAttendance(attendance.map(a => a.id === existing.id ? { ...a, checkin: manualIn, checkout: manualOut || null } : a));
    } else {
      const { data: row } = await supabase.from("attendance")
        .upsert({ employee_id: viewEmpId, date: manualDate, checkin: manualIn, checkout: manualOut || null }, { onConflict: "employee_id,date" })
        .select().single();
      if (row) setAttendance([...attendance, { ...row, employeeId: row.employee_id }]);
    }
  ...
  return (
    <>
      ...
      <div style={{ ...S.card, marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 16 }}>
          <div style={{ flex: 1, display: "flex", gap: 20 }}>
            ...
            {todayRecord?.checkin && todayRecord?.checkout && (
              <div><div style={S.statLabel}>Odpracováno</div><div style={{ fontSize: 22, fontWeight: 800, color: "#6366f1" }}>{fmtHours(calcHours(todayRecord.checkin, todayRecord.checkout))}</div></div>
            )}
          </div>
        ...
        {/* Zakázka a činnost pro dnešní záznam */}
        {todayRecord && (
          <div style={{ display: "flex", gap: 12, alignItems: "flex-end" }}>
            <div style={{ flex: 1 }}>
              <label style={S.label}>Zakázka</label>
              <select style={{ ...S.select, marginBottom: 0 }}
                value={todayRecord.project_id || ""}
                onChange={async e => {
                  const pid = e.target.value ? Number(e.target.value) : null;
                  await supabase.from("attendance").update({ project_id: pid }).eq("id", todayRecord.id);
                  setAttendance(attendance.map(a => a.id === todayRecord.id ? { ...a, project_id: pid } : a));
                }}>
            ...
            <div style={{ flex: 2 }}>
              <label style={S.label}>Provedená činnost</label>
              <input style={{ ...S.input, marginBottom: 0 }}
                placeholder="Co jsi dělal..."
                defaultValue={todayRecord.activity || ""}
                onBlur={async e => {
          ...
          {weekDates.map(d => {
            ...
            return (
              <div key={d} style={{ flex: 1, textAlign: "center" }}>
                ...
                <div style={{ height: 80, background: "#0a0d14", borderRadius: 8, border: `1px solid ${isToday ? "#6366f155" : "#1a2035"}`, display: "flex", flexDirection: "column", justifyContent: "flex-end", overflow: "hidden" }}>
                  {h > 0 && <div style={{ background: isToday ? "#6366f1" : "#34d399", height: `${Math.min((h / 10) * 100, 100)}%`, opacity: 0.7 }} />}
                </div>
                ...
                {r?.activity && <div style={{ fontSize: 9, color: "#6366f1", marginTop: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={r.activity}>📝 {r.activity}</div>}
              </div>
            );
          })}
        </div>
      ...
      <div style={S.card}>
        ...
        <table style={S.table}>
          <thead><tr>{["Datum", "Příchod", "Odchod", "Odprac.", "Zakázka", "Činnost", ""].map(h => <th key={h} style={S.th}>{h}</th>)}</tr></thead>
          <tbody>
              ...
              return (
                <>
                  <tr key={r.id} style={{ opacity: 1 }}>
                    ...
                    <td style={S.td}>{proj ? <span style={S.tag("#6366f1")}>{proj.name}</span> : <span style={{ color: "#334155" }}>—</span>}</td>
                    <td style={{ ...S.td, color: "#94a3b8", maxWidth: 200 }}>{r.activity || "—"}</td>
                    <td style={S.td}>
                      <div style={{ display: "flex", gap: 6 }}>
                        <button onClick={() => setEditRecord(isEditing ? null : { id: r.id, project_id: r.project_id, activity: r.activity || "" })}
                          style={{ background: "none", border: "none", color: "#6366f1", cursor: "pointer", fontSize: 14 }}>✏️</button>
                        {isHR && <button onClick={() => deleteRecord(r.id)} style={{ background: "none", border: "none", color: "#f87171", cursor: "pointer", fontSize: 15 }}>×</button>}
                      </div>
                    </td>
                  </tr>
                  {isEditing && (
                    <tr key={`edit-${r.id}`}>
                      <td colSpan={7} style={{ ...S.td, padding: "10px 12px" }}>
                        <div style={{ display: "flex", gap: 10, alignItems: "flex-end" }}>
                          <div style={{ flex: 1 }}>
                            <label style={S.label}>Zakázka</label>
                            <select style={{ ...S.select, marginBottom: 0 }} value={editRecord.project_id || ""}
                              onChange={e => setEditRecord({ ...editRecord, project_id: e.target.value ? Number(e.target.value) : null })}>
                              <option value="">— bez zakázky —</option>
                              {projects.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                            </select>
                          </div>
                          <div style={{ flex: 2 }}>
                            ...
                            <input style={{ ...S.input, marginBottom: 0 }} value={editRecord.activity}
                              onChange={e => setEditRecord({ ...editRecord, activity: e.target.value })}
                              placeholder="Popis činnosti..." />
                          </div>
                          <button style={{ ...S.btn(), padding: "9px 16px" }} onClick={() => saveRecordDetail(r.id, editRecord.project_id, editRecord.activity)}>Uložit</button>
                          <button style={{ ...S.btnGhost, padding: "9px 14px" }} onClick={() => setEditRecord(null)}>Zrušit</button>
                        </div>
                      </td>
                    </tr>
                  )}
